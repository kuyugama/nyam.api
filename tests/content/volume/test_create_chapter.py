...
async def test_without_title(client, token_admin, volume):
    index = 1
    title = None
    response = await requests.content.create_chapter(
        client, token_admin.body, volume.id, index, title
    )
    print(response.json())
...
async def test_title(client, token_admin, volume):
    index = 1
    title = "title"
    response = await requests.content.create_chapter(
        client, token_admin.body, volume.id, index, title
...
async def test_autoindex(client, token_admin, volume, chapter):
    index = None
    title = None
...
async def test_insert(client, token_admin, volume, chapter, session):
    index = chapter.index
    title = None
    response = await requests.content.create_chapter(
        client, token_admin.body, volume.id, index, title
